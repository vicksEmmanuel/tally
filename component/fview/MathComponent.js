...
import { NumberComponent } from "../svg/NumberComponent";
import Svg, { G, Path, Circle, Rect } from "react-native-svg";
import { MathScreen, ArithmeticsScreen } from "../screen";
...
export class MathComponent extends Component {
  ...
  questionControls() {
    const fraction = () => {
      let tops = this.props.math.count;
      let randomTopStatePicker = tops[Math.floor(Math.random() * 30) + 1];
      let randomTopStatePicker2 = tops[Math.floor(Math.random() * 30) + 1];
      let multiple = Math.floor(Math.random() * 20) + 2;
      let ques1 = "";
      const x = randomTopStatePicker.id * multiple;
      const y = randomTopStatePicker2.id * multiple;
      if (x < y) {
        ques1 = `${x}/${y}`;
      } else {
        ques1 = `${y}/${x}`;
      ...
      if (x / y === 1) {
        ...
        if (randomTopStatePicker.id < randomTopStatePicker2.id) {
          answer = `${randomTopStatePicker.id}/${randomTopStatePicker2.id}`;
        } else {
          answer = `${randomTopStatePicker2.id}/${randomTopStatePicker.id}`;
        }
      }
      return {
        question: `Simplify ${ques1}`,
        answer
      };
    };
    const plus = () => {
      ...
      let randomTopStatePicker2 = tops[Math.floor(Math.random() * tops.length)];

      let question = `Find the value ${randomTopStatePicker.id} + ${
        randomTopStatePicker2.id
      } `;
      let answer = `${randomTopStatePicker.id + randomTopStatePicker2.id}`;
      return {
        question,
        answer
      };
    ...
    const minus = () => {
      ...
      let question = "";
      if (randomTopStatePicker.id > randomTopStatePicker2.id) {
        question = `Find the value ${randomTopStatePicker.id} - ${
          randomTopStatePicker2.id
        }`;
      } else {
        question = `Find the value ${randomTopStatePicker2.id} - ${
          randomTopStatePicker.id
        }`;
      }
      let answer = "";
      if (randomTopStatePicker.id > randomTopStatePicker2.id) {
    ...
    const times = () => {
      let tops = this.props.math.count;
      let randomTopStatePicker = tops[Math.floor(Math.random() * 80)];
      let randomTopStatePicker2 = tops[Math.floor(Math.random() * 80)];
    ...
    const divide = () => {
      ...
      let jeep = {};
      if (randomTopStatePicker.id < randomTopStatePicker2.id) {
        jeep = deep(randomTopStatePicker2.id, randomTopStatePicker.id);
      } else {
        jeep = deep(randomTopStatePicker.id, randomTopStatePicker2.id);
      }
      let answer = "";
      if (jeep.remainder === 0) {
        answer = `${jeep.ans}`;
      } else {
        answer = `${jeep.ans} R ${jeep.remainder}`;
      }
      return {
        question,
        answer
    ...
    const letter = () => {
      let tops = this.props.math.count;
      let randomTopStatePicker = tops[Math.floor(Math.random() * tops.length)];
      let question = `${randomTopStatePicker.number}`;
      let answer = `${randomTopStatePicker.id}`;
      return {
        question,
        answer
      };
    ...
    const tally = () => {
      let tops = this.props.math.count;
      let randomTopStatePicker = tops[Math.floor(Math.random() * 30)];
      let question = `${randomTopStatePicker.id}`;
      let answer = `${randomTopStatePicker.id}`;
      return {
        question,
        answer
    ...
    const falseAnswers = sub => {
      if (sub === "fraction") {
        const rub = () => {
          ...
          let randomTopStatePicker = tops[Math.floor(Math.random() * 30) + 1];
          let randomTopStatePicker2 = tops[Math.floor(Math.random() * 30) + 1];
          let volume1 = randomTopStatePicker.id;
          let volume2 = randomTopStatePicker2.id;
          if (volume1 / volume2 === 1) {
            volume1 *= 5;
          }
          if (`${volume1}/${volume2}` === this.state.answer) {
            if (volume1 < volume2) {
              volume2 *= 5;
            } else {
              volume1 *= 5;
            }
          } else if (`${volume2}/${volume1}` === this.state.answer) {
            if (volume1 < volume2) {
              volume2 *= 5;
            } else {
              volume1 *= 5;
        ...
        const props = () => {
          let tops = this.props.math.count;
          let randomTopStatePicker =
            tops[Math.floor(Math.random() * tops.length)];
          let randomTopStatePicker2 =
            tops[Math.floor(Math.random() * tops.length)];
          let volume1 = randomTopStatePicker.id;
          let volume2 = randomTopStatePicker2.id;
          if (`${volume1 + volume2}` === this.state.answer) {
        ...
        const rub = () => {
          let tops = this.props.math.count;
          let randomTopStatePicker = tops[Math.floor(Math.random() * 100)];
          let randomTopStatePicker2 = tops[Math.floor(Math.random() * 100)];
          let volume1 = randomTopStatePicker.id;
          let volume2 = randomTopStatePicker2.id;
          if (`${volume1 - volume2}` === this.state.answer) {
            volume1 *= 5;
        ...
        let randomTopStatePicker2 =
          tops[Math.floor(Math.random() * tops.length)];
        if (`${randomTopStatePicker.id}` === this.state.answer) {
          randomTopStatePicker.id += 5;
        }
        if (`${randomTopStatePicker2.id}` === this.state.answer) {
          randomTopStatePicker2.id += 5;
        }
        if (`${randomTopStatePicker.id}` === `${randomTopStatePicker2.id}`) {
          randomTopStatePicker.id += 15;
        }
        return {
          false1: randomTopStatePicker.id,
          false2: randomTopStatePicker2.id
        };
      } else if (sub === "divide") {
        let tops = this.props.math.count;
        let randomTopStatePicker = tops[Math.floor(Math.random() * 30) + 1];
        let randomTopStatePicker2 = tops[Math.floor(Math.random() * 20) + 1];
        return {
          false1: `${randomTopStatePicker.id} R ${Math.floor(
            Math.random() * 30
          ) + 1}`,
          false2: `${randomTopStatePicker2.id} R ${Math.floor(
            Math.random() * 30
          ) + 1}`
        };
      } else if (sub === "tally") {
        let tops = this.props.math.count;
        let randomTopStatePicker = tops[Math.floor(Math.random() * 30)];
        let randomTopStatePicker2 = tops[Math.floor(Math.random() * 30)];
        let volume1 = randomTopStatePicker.id;
        let volume2 = randomTopStatePicker2.id;
        if (`${volume1}` === this.state.answer) {
          volume1 += 5;
        }
        if (`${volume2.id}` === this.state.answer) {
          volume2 += 5;
        }
        if (`${volume1}` === `${volume2}`) {
          volume2 += 15;
    ...
    if (this.state.view === null && this.state.expanded) {
      ...
      switch (randomQuestionPicker) {
        case "fraction":
          const topic = fraction();
          const question = topic.question;
          const answer = topic.answer;
          const falsify = falseAnswers("fraction");
          this.setState(
            {
              i: (this.state.i += 1),
              answer,
              subtopic: "Instruction: choose the simlified fraction",
              view: (
                <Text
                  numberOfLines={5}
                  style={{
        ...
        case "plus":
          ...
          const falsify2 = falseAnswers("plus");
          this.setState(
            {
              i: (this.state.i += 1),
              answer: answer2,
              subtopic: "Instruction: choose a value for the expression",
              view: (
                <Text
                  numberOfLines={5}
                  style={{
        ...
        case "minus":
          ...
          const falsify3 = falseAnswers("minus");
          this.setState(
            {
              i: (this.state.i += 1),
              answer: answer3,
        ...
        case "times":
          ...
          const falsify4 = falseAnswers("times");
          this.setState(
            {
              i: (this.state.i += 1),
              answer: answer4,
        ...
        case "tally":
          const topic7 = tally();
          const question7 = topic7.question;
          const answer7 = topic7.answer;
          const falsify7 = falseAnswers("tally");
          this.setState(
            {
              i: (this.state.i += 1),
              answer: answer7,
              subtopic: "Instruction: choose a number of the displayed tallies",
              view: (
                <View
                  style={{
                    width: "100%",
                ...
                >
                  {this.displayTally(Number(question7))}
                </View>
              )
            },
            () => {
              this.answerControls({
                answer: answer7,
                false1: falsify7.false1,
                false2: falsify7.false2
              });
            }
          );
          break;
        case "divide":
          const topic5 = divide();
          const question5 = topic5.question;
          const answer5 = topic5.answer;
          const falsify5 = falseAnswers("divide");
          this.setState(
            {
              i: (this.state.i += 1),
              answer: answer5,
            ...
            () => {
              this.answerControls({
                answer: answer5,
                false1: falsify5.false1,
                false2: falsify5.false2
              });
            }
          );
          break;
        case "letter":
          const topic6 = letter();
          const question6 = topic6.question;
          const answer6 = topic6.answer;
          const falsify6 = falseAnswers("letter");
          this.setState(
            {
              i: (this.state.i += 1),
              answer: answer6,
              subtopic: "Instruction: choose a number of the word",
              view: (
                <Text
                  numberOfLines={5}
                  style={{
            ...
            () => {
              this.answerControls({
                answer: answer6,
                false1: falsify6.false1,
                false2: falsify6.false2
              });
            }
          );
          break;
        default:
          const topic8 = letter();
          const question8 = topic8.question;
          const answer8 = topic8.answer;
          const falsify8 = falseAnswers("letter");
          this.setState(
            {
              i: (this.state.i += 1),
              answer: answer8,
            ...
            () => {
              this.answerControls({
                answer: answer8,
                false1: falsify8.false1,
                false2: falsify8.false2
              });
            }
          );
      }
    ...
    return (
      ...
      >
        <TouchableOpacity
          ...
          onPress={() => {
            this.props.navigation.navigate(MathScreen, {
              ...this.props,
              text: true
            });
          }}
        ...
        >
          ...
          >
            <NumberComponent number={Math.floor(Math.random() * 4) + 1} />
          </View>
          <View
            style={{
              flex: 1,
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            const data = this.props.math.count;
            const state = ["+", "-", "*", "/"];
            let newData = new Array(999);
            for (let i = 0; i < newData.length; i++) {
              let randomSign = Math.floor(Math.random() * state.length);
              let randomX = Math.floor(Math.random() * 1000);
              let randomY = Math.floor(Math.random() * 1000);
              if (state[randomSign] === "-") {
                if (data[randomX].id >= data[randomY].id) {
                  newData[i] = {
                    x: data[randomX].id,
                    y: data[randomY].id,
                    xName: data[randomX].number,
                    yName: data[randomY].number,
                    sign: `${state[randomSign]}`,
                    id: i,
                    ans: data[randomX].id - data[randomY].id
                  };
                } else {
                  newData[i] = {
                    x: data[randomY].id,
                    y: data[randomX].id,
                    xName: data[randomY].number,
                    yName: data[randomX].number,
                    sign: `${state[randomSign]}`,
                    id: i,
                    ans: data[randomX].id - data[randomY].id
                  };
                }
              } else if (state[randomSign] === "/") {
                ...
                if (data[randomX].id >= data[randomY].id) {
                  let jeep = {};
                  jeep = deep(data[randomX].id, data[randomY].id);
                  newData[i] = {
                    x: data[randomX].id,
                    y: data[randomY].id,
                    xName: data[randomX].number,
                    ...
                    ans:
                      jeep.remainder === 0
                        ? jeep.ans
                        : `${jeep.ans} r ${jeep.remainder}`
                  };
                } else {
                  let jeep = {};
                  jeep = deep(data[randomY].id, data[randomX].id);
                  newData[i] = {
                    x: data[randomY].id,
                    y: data[randomX].id,
                    xName: data[randomY].number,
            ...
            this.props.navigation.navigate(ArithmeticsScreen, {
              ...this.props,
              newData
            });
          }}
        ...
        <TouchableOpacity
          ...
          style={this.handleOnLongPress(
            this.state.fourth.name,
            this.state.fourth.long
          )}
          onPress={() => {
            this.changeLayout();
          }}
