...
export class MapComponent extends Component {
  ...
  questionControls() {
    const flag = () => {
      let tops = this.props.map.flag;
      let randomTopStatePicker = tops[Math.floor(Math.random() * tops.length)];
      let ques1 = index[randomTopStatePicker.code];
      let answer = randomTopStatePicker.country;
      return {
        question: ques1,
        answer
      };
    ...
    const continent = () => {
      let tops = this.props.map.flag;
      let randomTopStatePicker = tops[Math.floor(Math.random() * tops.length)];
      let ques1 = `What continent is ${randomTopStatePicker.country} in`;
      let answer = randomTopStatePicker.continent;
      return {
        question: ques1,
        answer
      };
    ...
    const falseAnswers = sub => {
      if (sub === "flag") {
        let tops = this.props.map.flag;
        let vXY = Math.floor(Math.random() * tops.length);
        let vXZ = Math.floor(Math.random() * tops.length);
        let randomTopStatePicker = tops[vXY];
        let randomTopStatePicker2 = tops[vXZ];
        if (randomTopStatePicker.country === this.state.answer) {
          if (vXY + 1 < top.length - 1) {
            randomTopStatePicker = tops[vXY + 1];
          } else {
            randomTopStatePicker = tops[vXY - 1];
          }
        }
        if (randomTopStatePicker2.country === this.state.answer) {
          if (vXZ + 1 < top.length - 1) {
            randomTopStatePicker2 = tops[vXZ + 1];
          } else {
            randomTopStatePicker2 = tops[vXZ - 1];
          }
        }
        if (randomTopStatePicker.country === randomTopStatePicker2.country) {
          if (vXZ + 1 < top.length - 1) {
            randomTopStatePicker2 = tops[vXZ + 1];
          } else {
            randomTopStatePicker2 = tops[vXZ - 1];
        ...
        return {
          false1: randomTopStatePicker.country,
          false2: randomTopStatePicker2.country
        };
      } else if (sub === "continent") {
        const continent = [
          "Africa",
          "Asia",
          "Europe",
        ...
        let x = Math.floor(Math.random() * continent.length);
        let y = Math.floor(Math.random() * continent.length);
        let dx = continent[x];
        let dy = continent[y];
        if (dx === dy) {
          if (x + 1 < continent.length - 1) {
            dx = continent[x + 1];
          } else {
            dx = continent[x - 1];
          }
        }
        if (dx === this.state.answer) {
          if (x + 1 < continent.length - 1) {
            dx = continent[x + 1];
          } else {
            dx = continent[x - 1];
        ...
        if (dy === this.state.answer) {
          if (dy + 1 < continent.length - 1) {
            dy = continent[x + 1];
          } else {
            dy = continent[x - 1];
          }
    ...
    if (this.state.view === null && this.state.expanded) {
      ...
      switch (randomQuestionPicker) {
        case "flag":
          ...
          const falsify = falseAnswers("flag");
          this.setState(
            {
              i: (this.state.i += 1),
              answer,
              subtopic: "Instruction: Choose the country's flag",
              view: (
                <View
                  style={{
                    flex: 1,
          ...
          break;
        case "continent":
          const topic3 = continent();
          const question3 = topic3.question;
          const answer3 = topic3.answer;
          const falsify3 = falseAnswers("continent");
          this.setState(
            {
              i: (this.state.i += 1),
              answer: answer3,
        ...
        default:
          ...
          const falsify2 = falseAnswers("flag");
          this.setState(
            {
              i: (this.state.i += 1),
              answer: answer2,
              ...
              view: (
                ...
                >
                  ...
                  <Image source={question2} style={{ width: 30, height: 30 }} />
                </View>
              )
            },
            () => {
    ...
    return (
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            this.props.navigation.navigate(FlagScreen, { ...this.props });
          }}
          style={this.handleOnLongPress(
